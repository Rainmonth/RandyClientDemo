...
public class DownloadManager {
    ...
    public boolean writeResponseBodyToDisk(final String key, String path, String name,
                                           Context context, ResponseBody body) {
        ...
        if (type.contains(APK_CONTENT_TYPE)) {
            ...
            fileSuffix = ".png";
        } else if (type.contains(JPG_CONTENT_TYPE)) {
            fileSuffix = ".jpg";
        }
        ...
        if (path == null) {
            path = context.getExternalFilesDir(null) + File.separator + "DownLoads"
                    + File.separator;
        }
            ...
            try {
              ...
              /*  while (true) {
                ...
                if (callBack != null) {
                    ...
                    Log.d(TAG, "file downloaded: " + fileSizeDownloaded + " of " + fileSize);
                    Log.d(TAG, "file downloaded: is success");
                }
                return true;
            } catch (IOException e) {
                finalOnError(e);
    ...
    private void finalOnError(final Exception e) {
        ...
        if (Utils.checkMain()) {
            callBack.onError(new SelfDefineThrowable(100, e));
        } else {
            handler.post(new Runnable() {
                @Override
                public void run() {
